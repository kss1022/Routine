...
struct Rest: ValueObject{
    ...
    init(_ command: TimeSectionCommand) throws{
        if command.name.count > 50{ throw ArgumentException("Rest's name Length must less then 50")  }
        
        if command.description.count > 50{ throw ArgumentException("Rest's descriptoin Length must less then 50")  }
    ...
    private enum CodingKeys: String{
        case name = "RestName"
        case description = "RestDescription"
        case min = "RestMin"
        case sec = "RestSec"
        case emoji = "RestEmoji"
        case tint = "RestTint"
    }
}

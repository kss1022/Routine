...
struct Exercise: ValueObject{
    ...
    init(_ command: TimeSectionCommand) throws{
        if command.name.count > 50{ throw ArgumentException("Exercise's name Length must less then 50")  }
        
        if command.description.count > 50{ throw ArgumentException("Exercise's descriptoin Length must less then 50")  }
    ...
    private enum CodingKeys: String{
        ...
        case description = "ExerciseDescription"
        case min = "ExerciseMin"
        case sec = "ExerciseSec"
        case emoji = "ExerciseEmoji"
        case tint = "ExerciseTint"
    }
}

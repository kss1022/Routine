...
struct Round: ValueObject{
    ...
    init(_ command: RepeatSectionCommand) throws{
        if command.name.count > 50{ throw ArgumentException("Round's name Length must less then 50")  }
        
        if command.description.count > 50{ throw ArgumentException("Round's descriptoin Length must less then 50")  }
    ...
    private enum CodingKeys: String{
        case name = "RoundName"
        case description = "RoundDescription"
        case `repeat` = "RoundRepeat"
        case emoji = "RoundEmoji"
        case tint = "RoundTint"
    }
}

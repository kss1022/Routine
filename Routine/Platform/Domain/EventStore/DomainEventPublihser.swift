...
public final class DomainEventPublihser{
    ...
    func publish<T: DomainEvent>(_ domainEvent: T) async{        
    ...
    @discardableResult
    public func onReceive<E>(_ eventType: E.Type, perform action: @escaping (E) -> Void) -> AnyCancellable where E: DomainEvent {
        let listener: EventListener = getOrCreateEventListener(for: E.self)
        return listener.registerSubscription(action: action)
    }
    
    @discardableResult
    public func onReceive<E: DomainEvent, S: Scheduler>(_ eventType: E.Type, performOn scheduler: S, action: @escaping (E) -> Void) -> AnyCancellable {
        let listener: EventListener = getOrCreateEventListener(for: E.self)
...
private extension DomainEventPublihser {
    func getOrCreateEventListener<E: DomainEvent>(for eventType: E.Type) -> EventListener {

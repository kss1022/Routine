...
struct Ready: ValueObject{
    ...
    init(_ command: TimeSectionCommand) throws{
        if command.name.count > 50{ throw ArgumentException("Ready's name Length must less then 50")  }
        
        if command.description.count > 50{ throw ArgumentException("Ready's descriptoin Length must less then 50")  }
    ...
    private enum CodingKeys: String{
        case name = "ReadyName"
        case description = "ReadyDescription"
        case min = "ReadyMin"
        case sec = "ReadySec"
        case emoji = "ReadyEmoji"
        case tint = "ReadyTint"
    }
}

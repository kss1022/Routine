...
public final class EmojiPickerViewController: UIViewController {
    ...
    private var viewModel: EmojiPickerViewModel
...
extension EmojiPickerViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.setEmoji(with: viewModel.emoji(at: indexPath))
        return cell
    }
    
    public func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        
        guard  let header = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: "EmojiCollectionViewHeader", for: indexPath) as? EmojiHeaderView else {
            fatalError("Failed to dequeue reusable cell")
        }
        
        header.setTitle(with: viewModel.sectionHeader(for: indexPath.section))
        return header
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let emoji = viewModel.emoji(at: indexPath)
...
extension EmojiPickerViewController: UIScrollViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if let selectedEmojiCategoryIndex = indexPathsForVisibleHeaders.first?.section,
           viewModel.selectedEmojiCategoryIndex != selectedEmojiCategoryIndex {
            viewModel.selectedEmojiCategoryIndex = selectedEmojiCategoryIndex
            
            self.emojiPickerView.updateSelectedCategoryIcon(with: selectedEmojiCategoryIndex)
        }
    }
}
...
extension EmojiPickerViewController: EmojiPickerViewDelegate {
    ...
    func didChoiceEmojiCategory(at index: Int) {
        feedbackGenerator?.impactOccurred()
        viewModel.selectedEmojiCategoryIndex = index
    }
}

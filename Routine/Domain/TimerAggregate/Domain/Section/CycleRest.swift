...
struct CycleRest: ValueObject{
    ...
    init(_ command: TimeSectionCommand) throws{
        if command.name.count > 50{ throw ArgumentException("CycleRest's name Length must less then 50")  }
        
        if command.description.count > 50{ throw ArgumentException("CycleRest's descriptoin Length must less then 50")  }
    ...
    private enum CodingKeys: String{
        case name = "CycleRestName"
        case description = "CycleRestDescription"
        case min = "CycleRestMin"
        case sec = "CycleRestSec"
        case emoji = "CycleRestEmoji"
        case tint = "CycleRestTint"
    }
}
